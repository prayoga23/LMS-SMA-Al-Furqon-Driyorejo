...
export default function EditSubjectPage() {
  ...
  const fetchSubject = async () => {
    try {
      setLoading(true);
      const res = await api.get(`/subjects/${subId}`);
      const s = res.data;
      setFormData({
        code: s.code || '',
        name: s.name || '',
        category: s.category || 'Wajib A',
        kkm: s.kkm !== undefined ? s.kkm : 75,
        jp: s.jp !== undefined ? s.jp : 3,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.put(`/subjects/${subId}`, formData);
      router.push('/admin/subjects');
    } catch (err: any) {
      setError(err.response?.data?.message || 'Gagal memperbarui data mata pelajaran.');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="max-w-4xl mx-auto space-y-6 animate-fade-in pb-12">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Perbarui kode, nama, standar KKM, alokasi jam pelajaran, atau status mata pelajaran
          </p>
        </div>
