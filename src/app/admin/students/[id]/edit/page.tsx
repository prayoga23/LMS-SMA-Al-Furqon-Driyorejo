  ...
  const fetchStudent = async () => {
    try {
      ...
      setFormData({
        nis: s.nis || '',
        name: s.name || '',
        class: s.class || 'X IPA 1',
        major: s.major || 'IPA',
        entry_year: String(s.entryYear || s.entry_year || 2024),
        parent_name: s.parent?.user?.name || '',
        parent_phone: s.parent?.phone || '',
        is_santri: Boolean(s.isSantri ?? s.is_santri ?? false),
        residence_type: s.residenceType || s.residence_type || 'Non-Asrama',
        spp_nominal: String(s.sppNominal ?? s.spp_nominal ?? 500000),
        activity_nominal: String(s.activityNominal ?? s.activity_nominal ?? 150000),
        has_discount: Boolean(s.hasDiscount ?? s.has_discount ?? false),
        discount_notes: s.discountNotes || s.discount_notes || '',
      });
    } catch (err) {
      console.error(err);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              Perbarui data akademik, status santri, dan kontak wali siswa SMA Al-Furqon Driyorejo
            </p>
          </div>
        </div>
