...
export default function ImportStudentsPage() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const autoMap = autoMatchColumns(headers, STUDENT_TARGET_FIELDS);
  ...
  const handleGoToPreview = () => {
    setError('');
    const requiredUnmapped = STUDENT_TARGET_FIELDS.filter(
    ...
    const mapped = mapExcelRowsToSchema(rawRows, columnMapping, STUDENT_TARGET_FIELDS);
  ...
  const handleExecuteImport = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Terjadi kesalahan saat mengimport data ke server.');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="max-w-5xl mx-auto space-y-6 animate-fade-in pb-12">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              Unggah file Excel (.xlsx / .csv) untuk menambah atau memperbarui data siswa secara kolektif
            </p>
          </div>
        ...
        {step === 1 && (
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-8 text-center space-y-6">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-800">Pilih File Excel (.xlsx / .xls / .csv)</h3>
              <p className="text-xs text-slate-500 mt-1">
                Pastikan baris pertama pada lembar kerja berisi nama kolom (header)
              </p>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-6 space-y-6">
            ...
            <div className="space-y-4">
              <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider flex items-center gap-2">
                ...
                Pemetaan Kolom Excel ke Data Sistem ({rawRows.length} baris terbaca)
              </h3>

              <div className="border border-slate-200 rounded-xl overflow-hidden divide-y divide-slate-100 text-xs">
                {STUDENT_TARGET_FIELDS.map((field: any) => {
        ...
        {step === 3 && (
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-6 space-y-6">
            <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider flex items-center gap-2">
              ...
              Pratinjau {mappedData.length} Data Siswa Siap Diimport
            </h3>
              ...
              <table className="w-full text-left text-xs text-slate-700">
                <thead className="bg-slate-100 uppercase text-[10px] text-slate-700 font-bold border-b border-slate-200 sticky top-0">
                  <tr>
                    ...
                    <th className="py-3 px-3">Status Santri</th>
                    <th className="py-3 px-3">Wali / Email</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 font-medium">
                  {mappedData.map((row, idx) => (
                    <tr key={idx} className="hover:bg-slate-50">
                      ...
                      <td className="py-2.5 px-3 font-mono font-bold text-slate-900">{row.nis || '-'}</td>
                      <td className="py-2.5 px-3 font-bold text-slate-900">{row.name || '-'}</td>
                      <td className="py-2.5 px-3">{row.class || 'X IPA 1'}</td>
                      <td className="py-2.5 px-3">{row.major || 'IPA'}</td>
                      <td className="py-2.5 px-3">{row.entry_year || 2024}</td>
                      <td className="py-2.5 px-3">{row.is_santri ? 'Santri' : 'Non-Santri'}</td>
                      <td className="py-2.5 px-3 text-slate-500">
                        {row.parent_name || 'Wali'} ({row.parent_email || 'ortu@sekolah.sch.id'})
                      </td>
                    </tr>
                  ))}
                </tbody>
              ...
              >
                {submitting ? (
                  <>
                    ...
                    <span>Mengimport Data Siswa...</span>
                  </>
                ) : (
                  <>
                    <CheckCircle2 className="w-4 h-4" />
                    <span>Konfirmasi Import {mappedData.length} Siswa</span>
                  </>
                )}
              </button>
            </div>
